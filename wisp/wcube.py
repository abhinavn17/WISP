...
import subprocess
from wisp.cube_join import cube_join
import glob
import sys
...
def main():
    ...
    outdir = f'{workdir}/{msfilename}_cube'
    ...
    for file in glob.glob(f'{outdir}/*-residual.fits'):
        os.remove(file)
    for file in glob.glob(f'{outdir}/*-tmp.fits'):
        os.remove(file)
    for file in glob.glob(f'{outdir}/*MFS*'):
        os.remove(file)

    cube_join(outdir, f'{workdir}/{msfilename}_cube.fits')
